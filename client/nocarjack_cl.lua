	...
	while true do
		...
		if DoesEntityExist(GetVehiclePedIsTryingToEnter(PlayerPedId())) then
			...
			local lock = GetVehicleDoorLockStatus(veh)
                ...
                if IsVehicleModel(veh, GetHashKey('scorcher')) then
                lucky= true
                end
				...
				local pedd = GetPedInVehicleSeat(veh, -1)
				local plate = GetVehicleNumberPlateText(veh)
                if IsVehicleModel(veh, GetHashKey('cruiser')) then
                lucky= true
                end
				...
				local pedd = GetPedInVehicleSeat(veh, -1)
				local plate = GetVehicleNumberPlateText(veh)
                if IsVehicleModel(veh, GetHashKey('tribike')) then
                lucky= true
                end
				...
				local pedd = GetPedInVehicleSeat(veh, -1)
				local plate = GetVehicleNumberPlateText(veh)
                if IsVehicleModel(veh, GetHashKey('tribike2')) then
                lucky= true
                end
				...
				local pedd = GetPedInVehicleSeat(veh, -1)
				local plate = GetVehicleNumberPlateText(veh)
                if IsVehicleModel(veh, GetHashKey('blazer')) then
                lucky= true
                end
